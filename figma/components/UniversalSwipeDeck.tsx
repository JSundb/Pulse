...
export default function UniversalSwipeDeck({
  ...
  const thirdActivity = activities[currentIndex + 2];
  ...
  const handleSwipeRight = () => {
    ...
    if (!savedActivityIds.includes(currentActivity.id)) {
      setSavedActivityIds([...savedActivityIds, currentActivity.id]);
    }
    if (currentIndex < activities.length - 1) {
      setCurrentIndex(currentIndex + 1);
    }
    ...
    return (
      <div className="flex h-full items-center justify-center p-8 text-center">
        <div>
          ...
          <h2 className="mb-2 text-2xl text-foreground">{emptyMessage}</h2>
          <p className="text-muted-foreground">Check back later for new spots and activities</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative flex h-full items-center justify-center px-4 pb-4 pt-16">
      {/* Instructional hint */}
      <div className="absolute top-2 left-0 right-0 z-50 flex justify-center px-4">
        <div className="rounded-full bg-secondary px-4 py-2 shadow-sm">
          <p className="text-xs text-muted-foreground">
            Swipe right to save • Swipe left to pass • Swipe up for details
          </p>
        </div>
      </div>

      <div className="relative w-full max-w-md h-full">
        {/* Card Stack - render in reverse order so z-index works correctly */}
        
        {/* Third Card (furthest back) */}
        {thirdActivity && (
          <TinderSwipeCard
            key={thirdActivity.id}
