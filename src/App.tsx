...
import { Home, Heart, MapPin, MessageCircle, Bell, Compass } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [activitiesData, themesData, userData] = await Promise.all([
          api.getActivities(),
          api.getThemes(),
          api.getCurrentUser()
        ]);
        ...
        toast.success(`Loaded ${activitiesData.length} activities!`);
      } catch (error) {
        console.error('❌ Error fetching data:', error);
        toast.error('Failed to load data. Make sure API server is running on port 3001');
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="flex h-screen w-full max-w-[480px] flex-col overflow-hidden bg-background shadow-2xl">
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {!viewingActivity && !showActivityChat && !showSearch && activeTab === 'saved' && (
            <SavedList 
              onOpenActivity={(activity) => setViewingActivity(activity)} 
            />
          )}
